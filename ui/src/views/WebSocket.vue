    ...
    <div class="">
        ...
        <template v-for="user in users">
            <div class="border" style="margin-bottom: 20px;">
                <div class="pd10">PEER: {{ user.device }}-{{ user.tempName }}-{{ user.id }}-{{user.online}}
                <div>
                    <el-button type="danger" size="mini" @click="remove_user(user)"><i class="el-icon-delete"></i></el-button>
                </div>
                </div>
...
import {add_msg_to_user_by_id, add_user, delete_user_by_id, get_all_users, get_me, set_me} from "@/storage";
        ...
        this.webSocketClient.onmessage = (message) => {
            ...
            if (isjson) {
                ...
                if (type === "HELLO_BACK") {
                    ...
                    let users_from_storage = get_all_users()
                    ...
                    // filter me
                    b.user_list = b.user_list.filter((it: any) => it.id !== this.me.id)

                    localStorage.setItem("users", JSON.stringify(users_from_storage))
